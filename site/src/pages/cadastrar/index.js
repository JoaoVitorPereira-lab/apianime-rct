import {alterarAnime, cadastrarAnime} from '../../api/filmeApi';
...
export default function Index() {
    ...
    async function salvarClick(){
        ...
        {
            if(id === 0){
                const novoFilme = await cadastrarAnime(nome);
                setId(novoFilme.id);
                toast.dark('🔥 Filme cadastrado com sucesso!');
            }
            else{
                toast.dark('Clique em novo para um novo cadastro ou aperte em consultar para ver todos os animes');
            }
        }
        catch(err)
        {
